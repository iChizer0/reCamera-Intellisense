...
BIN_DIR = Path.home() / ".recamera" / "bin"
LAUNCHER_PATH = BIN_DIR / "rci"
_RC_BEGIN = "# >>> recamera-intellisense >>>"
_RC_END = "# <<< recamera-intellisense <<<"
...
class Ui:
    ...
    def warn(cls, message: str) -> None:
        print(f" {cls.yellow('!')} {message}", file=sys.stderr)
    ...
    try:
        with urlopen(req, timeout=DOWNLOAD_TIMEOUT) as response:
            ...
            if total > MAX_DOWNLOAD_BYTES:
                raise SetupError(
                    f"Source archive too large to download safely ({_format_mb(total)} > {_format_mb(MAX_DOWNLOAD_BYTES)})."
                )
            received = 0
            last_pct = -1
            with open(dest, "wb") as handle:
                while True:
                    ...
                    if received > MAX_DOWNLOAD_BYTES:
                        raise SetupError(
                            f"Source archive exceeded {_format_mb(MAX_DOWNLOAD_BYTES)} during download."
                        )
...
def _launcher_text(scripts_dir: Path) -> str:
    return (
        "#!/bin/sh\n"
        "# reCamera Intellisense CLI (generated by setup-skill.py)\n"
        f"PYTHONPATH={shlex.quote(str(scripts_dir))} exec python3 -m recamera_intellisense \"$@\"\n"
    )


def install_launcher(skill_dir: Path) -> Path:
    BIN_DIR.mkdir(parents=True, exist_ok=True)
    LAUNCHER_PATH.write_text(_launcher_text(skill_dir / "scripts"), encoding="utf-8")
    LAUNCHER_PATH.chmod(0o755)
    return LAUNCHER_PATH


def remove_launcher(removed_skill_dirs: Sequence[Path]) -> bool:
    """Remove the launcher only when it points at one of the removed skills."""
    if not LAUNCHER_PATH.is_file():
        return False
    target = str(LAUNCHER_PATH.read_text(encoding="utf-8"))
    if not any(str(d / "scripts") in target for d in removed_skill_dirs):
        return False
    LAUNCHER_PATH.unlink()
    try:
        BIN_DIR.rmdir()  # fails (harmlessly) when non-empty, e.g. MCP binary present
    except OSError:
        pass
    return True
...
def _rc_files() -> list[Path]:
    return [p for p in (Path.home() / ".bashrc", Path.home() / ".zshrc") if p.is_file()]


def _add_path_block(rc: Path) -> bool:
    text = rc.read_text(encoding="utf-8")
    if _RC_BEGIN in text:
        return False
    with open(rc, "a", encoding="utf-8") as fh:
        if text and not text.endswith("\n"):
            fh.write("\n")
        fh.write(f'{_RC_BEGIN}\nexport PATH="$HOME/.recamera/bin:$PATH"\n{_RC_END}\n')
    return True


def _remove_path_block(rc: Path) -> bool:
    lines = rc.read_text(encoding="utf-8").splitlines(keepends=True)
    out: list[str] = []
    i = 0
    changed = False
    while i < len(lines):
        if lines[i].strip() == _RC_BEGIN:
            while i < len(lines) and lines[i].strip() != _RC_END:
                i += 1
            i += 1  # skip the end marker (or the unterminated tail)
            changed = True
        else:
            out.append(lines[i])
            i += 1
...
def do_uninstall(args: argparse.Namespace) -> int:
    ...
    for candidate in targets:
        ...
        try:
            ...
            removed_any = True
            removed_dirs.append(candidate.paths.skill_dir)
        except OSError as exc:
            Ui.warn(f"Could not remove {candidate.paths.skill_dir}: {exc}")
    if remove_launcher(removed_dirs):
        Ui.info(f"Removed {LAUNCHER_PATH}")
        for rc in _rc_files():
            if _remove_path_block(rc):
                Ui.info(f"Removed PATH block from {rc}")
    print()
    if removed_any:
...
def normalise_argv(argv: list[str]) -> list[str]:
    ...
    while i < len(argv):
        ...
        if token in _VALUE_FLAGS:
            i += 2
            continue
        if any(token.startswith(f"{flag}=") for flag in _VALUE_FLAGS):
            i += 1
            continue
        if not token.startswith("-"):
            positionals.append(token)
        i += 1
    if not positionals or positionals[0] not in SUBCOMMANDS:
        return ["install", *argv]
    return argv
